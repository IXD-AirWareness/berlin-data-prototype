...
//fun String.toPeriod() = Period.values().find { it.csvName == this } ?: throw RuntimeException("Could not parse '$this' to Period.")
    ...
    enum class Pollutant(val csvName: String) {
        PM10("Feinstaub (PM10)"),
    ...
    fun String.toPollutant(): Pollutant {
        return Pollutant.values().find { it.csvName == this }
            ?: throw RuntimeException("Could not parse '$this' to Pollutant.")
    }
    ...
    private fun parsePollutionCSV(csv: String, type: Type): Data {
        ...
        val pollutant = lines.elementAt(1).split(";")[1].toPollutant()
        ...
        return Data.Pollution(pollutant, unit, type, stations, timedValues)
    }
    ...
    private fun getPollutionCSV(
        ...
        val query =
            "https://luftdaten.berlin.de/core/${pollutant.name.toLowerCase(Locale.ROOT)}.csv?stationgroup=${type.name}&period=${period.param}" +
        ...
        when (result) {
            ...
            is Result.Success -> {
                if (saveToFile) {
                    ...
                    val path =
                        Paths.get("./data/${startDate.year}/${pollutant}_${type}_${period}_$startDate-${startHour}_$endDate-$endHour.csv")
